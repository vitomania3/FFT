function res_struct = plotFT(hFigure, fHandle, fFTHandle, step, inpLimVec, outLimVec)
...
if (isempty(SPlotInfo)) 
    ...
    set(real_fourier_plot, 'XLim', outLimVec);
    set(imag_fourier_plot, 'XLim', outLimVec);
...
if (isempty(outLimVec))
    ...
    set(SPlotInfo.real, 'XLim', outLimVec);
    set(SPlotInfo.imag, 'XLim', outLimVec);
end
set(hFigure, 'UserData', SPlotInfo);
res_struct.outLimVec = outLimVec;

final_l_splitting = l_splitting - outLimVec(1);
...
final_l_splitting = l_splitting - outLimVec(2);
final_l_splitting = final_l_splitting(2 : end) .* final_l_splitting(1 : (end - 1));
...
if (~isempty(fFTHandle))
    ...
    legend('real fourier', 'real analytic fourier');
end
...
if (~isempty(fFTHandle))
    ...
    legend('image fourier', 'image analytic fourier');
end
end